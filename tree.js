...
function DrawTree() {
  ...
  for (let i = 0; i < node_array.length; ++i) {
    ...
    if (node_array[i].left !== null) {
        DrawEdge(node_array[i].x, node_array[i].y, node_array[i].left.x, node_array[i].left.y, node_array[i].left_color);
    }
    if (node_array[i].right !== null) {
        DrawEdge(node_array[i].x, node_array[i].y, node_array[i].right.x, node_array[i].right.y, node_array[i].right_color);
    }
  }
}
...
function UndrawTree() {
    for (let i = 0; i < node_array.length; ++i) {
        ...
        if (node_array[i].left !== null) {
            UndrawEdge(node_array[i].x, node_array[i].y, node_array[i].left.x, node_array[i].left.y, node_array[i].left_color);
        }
        if (node_array[i].right !== null) {
            UndrawEdge(node_array[i].x, node_array[i].y, node_array[i].right.x, node_array[i].right.y, node_array[i].right_color);
        }
      }
}
...
function CopyState() {
    ...
    state_array.push(copyArray);
    step_messages.push(current_message);
}
...
function Merge(a, b) {
    ...
    if (a !== null && b !== null) {
        current_message = "Current step: " + "Merging recursively (" + a.x + ";" + a.y + ") and " + "(" + b.x + ";" + b.y + ").";
    } else if (a === null) {
        current_message = "Current step: " + "Merging (" + b.x + ";" + b.y + ")" + " with empty tree, returning from recursion base case.";
    } else if (b === null) {
        current_message = "Current step: " + "Merging (" + a.x + ";" + a.y + ")" + " with empty tree, returning from recursion base case.";
    } else {
        current_message = "Current step: " + "Merging empty trees, returning from recursion base case.";
    }
    CopyState();
    if (a === null) {
        if (b !== null) {
    ...
    if (a.y >= b.y) {
        ...
        if (a.right !== null) {
            a.right.parent = a;
            current_message = "Current step: " + "Going back in recursion, adding new edge: (" + a.x + ";" + a.y + ") -> " + "(" + a.right.x + ";" + a.right.y + ").";
        }
        a.right_color = blue;
        CopyState();
        a.right_color = black;
        ...
        if (b.left !== null) {
            b.left.parent = b;
            current_message = "Current step: " + "Going back in recursion, adding new edge: (" + b.x + ";" + b.y + ") -> " + "(" + b.left.x + ";" + b.left.y + ").";
        }
        b.left_color = blue;
        CopyState();
        b.left_color = black;
...
function Split(root, key) {
    ...
    current_message = "Current step: " + "Splitting recursively " + "(" + root.x + ";" + root.y + ").";
    CopyState();
    if (root.x <= key) {
        root.right_color = red;
        root.node_color = yellow;
        current_message = "Current step: " + "Root key is smaller than split key, so going recursively to right subtree.";
        CopyState();
        ...
        if (!was_null) {
            current_message = "Current step: " + "Going back in recursion";
            if (root.right !== null) {
                current_message = "Current step: " + "Going back in recursion, adding new edge: (" + root.x + ";" + root.y + ") -> " + "(" + root.right.x + ";" + root.right.y + ").";
            }
            CopyState();
        }
        ...
        root.left_color = red;
        root.node_color = yellow;
        current_message = "Current step: " + "Root key is bigger than split key, so going recursively to left subtree.";
        CopyState();
        ...
        if (!was_null) {
            current_message = "Current step: " + "Going back in recursion";
            if (root.left !== null) {
                current_message = "Current step: " + "Going back in recursion, adding new edge: (" + root.x + ";" + root.y + ") -> " + "(" + root.left.x + ";" + root.left.y + ").";
            }
            CopyState();
        }
...
async function PlayButton() {
    ...
    for (let i = current_state; i < state_array.length; ++i) {
        ...
        current_state = i;
        UndrawTree();
        node_array = state_array[i];
        current_message = step_messages[i];
        DrawTree();
        await Wait(1500);
    }
}
...
async function NextStepButton() {
    ...
    UndrawTree();
    node_array = state_array[current_state];
    current_message = step_messages[current_state];
    DrawTree();
}
...
function GoToLastStep() {
    ...
    if (state_array.length > 0) {
        UndrawTree();
        node_array = state_array[state_array.length - 1];
        current_message = step_messages[state_array.length - 1];
        DrawTree();
        state_array = [];
        step_messages = [];
        return true;
...
async function BuildInsertButton() {
    ...
    if (key < 1 || key > grid_cell_width || priority < 1 || priority > grid_cell_height) {
...
async function BuildDeleteButton() {
    ...
    ErrorHandler("Node with such Key and Priority doesn't exits in current build operation");
}
...
async function BuildButton() {
    ...
    for (let i = 0; i < cur_tree.length; ++i) {
        for (let j = 0; j < node_array.length; ++j) {
            if (node_array[j].x === cur_tree[i].x && node_array[j].y === cur_tree[i].y) {
...
async function ClearAllButton() {
    ...
    step_messages.splie(0, step_messages.length);
    DrawTree();
}
...
async function MergeButton() {
    ...
    if (GetMaxKey(a) >= GetMinKey(b)) {
        ErrorHandler("The first tree in merge should have keys less then every key in the second tree!");
        return;
    }
    UndrawTree();
    Merge(a, b);
...
async function SplitButton() {
    ...
    if (isNaN(root_key) || isNaN(split_key) || root_key % 1 != 0 || split_key % 1 != 0) {
...
async function InsertButton() {
    ...
    let priority = Number(document.getElementById("insert-delete-priority").value);
    let root_key = Number(document.getElementById("insert-delete-root-key").value);
    if (isNaN(key) || isNaN(priority) || isNaN(root_key) || key % 1 != 0 || priority % 1 != 0 || root_key % 1 != 0) {
...
async function DeleteButton() {
    ...
    CopyState();
    if (split_result.second === null) {
        step_messages.splice(state_count, state_array.length - state_count);
    ...
    for (let i = 0; i < node_array.length; ++i) {
        if (node_array[i].x === second_split_result.second.x && node_array[i].y === second_split_result.second.y) {
...
async function BuildRandomTreeButton() {
    ...
    for (let i = 0; i < random_priorities.length; ++i) {
        if (Math.floor(Math.random() * 100) <= randomness) {
            ...
            [random_priorities[swap_index], random_priorities[i]] = [random_priorities[i], random_priorities[swap_index]];
        }
    }
